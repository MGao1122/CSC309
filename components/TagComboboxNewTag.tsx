...
export function TagComboboxNewTag({ selectedTags, setSelectedTags }: TagComboboxProps) {
    ...
    const handleNewTagCreation = (e: React.KeyboardEvent) => {
        if (e.key === "Enter" && tagQuery.trim()) {
            e.preventDefault()
            const newTag = tagQuery.trim()

            if (!selectedTags.includes(newTag) && !availableTags.some(tag => tag.name === newTag)) {
                setSelectedTags([...selectedTags, newTag])
                setAvailableTags([...availableTags, { name: newTag }])
            }
            setTagQuery("")
            setOpen(false)
        }
            ...
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    <Button
                        ...
                        role="combobox"
                        aria-expanded={open}
                        className="w-[250px] justify-between bg-white/30 dark:bg-black/30 border border-gray-800/30 dark:border-gray-400/30 text-gray-700 hover:bg-white/50 dark:text-gray-300 dark:hover:bg-gray-800/50 transition-colors duration-500 transition-ease-in-out"
                    >
                        Select or create tags...
                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                    </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[250px] p-0 popover-content">
                    <Command>
                        <CommandInput
                            ...
                            onKeyDown={handleNewTagCreation} // Detect "Enter" to create new tag
                        />
                        <CommandList>
