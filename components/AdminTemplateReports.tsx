...
const AdminTemplateReports = () => {
    ...
    const fetctReports = async () => {
        try {
            const { data } = await api.get(`/admin/template?page=${currentPage}&perPage=${perPage}&showHidden=${!showHidden}`);
    ...
    const handlePageClick = (page: number) => {
        if (page >= 1 && page <= totalPages) {
            ...
            router.push({
                ...
                query: { ...query, templatesPage: page.toString() }
    ...
    return (
        <div className="container mx-auto p-4">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold">Template Reports</h1>
                <Toggle
                    pressed={showHidden}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4 mt-2">
                {templates.map((comment) => (
                    <Link href={`/admin/template/${comment.id}?reportId=${comment.report[0].id}`} key={comment.id}>
                        <FrostedGlass key={comment.id} className={`h-48 flex flex-col break-words overflow-hidden cursor-pointer hover:bg-white/60 dark:hover:bg-black/60 transition-colors duration-500 transition-ease-in-out`}>
                            <CardHeader className="p-2 break-word">
                                <h4>{comment.title.slice(0, 30)}{comment.title.length > 30 ? '...' : ''}</h4>
                            </CardHeader>
            ...
            {templates.length === 0 && <p>No comments to show</p>}
        </div>
    );

}

export default AdminTemplateReports;