...
const AdminPostsReports = () => {
    ...
    const fetctReports = async () => {
        try {
            const { data } = await api.get(`/admin/post?page=${currentPage}&perPage=${perPage}&showHidden=${!showHidden}`);
    ...
    const handlePageClick = (page: number) => {
        if (page >= 1 && page <= totalPages) {
            ...
            router.push({
                ...
                query: { ...query, postPage: page.toString() }
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4 mt-2">
                {posts.map((comment) => (
                    <Link href={`/admin/post/${comment.id}?reportId=${comment.report[0].id}`} key={comment.id}>
...
export default AdminPostsReports;