...
const PostList: React.FC = () => {
    ...
    const fetchPosts = async (page: number) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/posts?page=${page}&perPage=8`);
            if (!response.ok) {
    ...
    useEffect(() => {
        const initialPage = query.postPage ? parseInt(query.postPage as string) : 1; // Use distinct query parameter
        setCurrentPage(initialPage);
        fetchPosts(initialPage);
    }, [query.postPage]);

    // Handle setting the page when a page link is clicked
    const handlePageClick = (page: number) => {
        if (page >= 1 && page <= totalPages) {
            setCurrentPage(page);
            router.push({
                pathname: router.pathname,
                query: { ...query, postPage: page.toString() }, // Use distinct query parameter
    ...
    return (
        <div className="container mx-auto p-4">
            <div className="flex justify-between">
                ...
                <Button variant="outline" onClick={() => router.push('/posts')} className='bg-white/30 dark:bg-black/30 border border-gray-800/30 dark:border-gray-400/30 text-gray-700 hover:bg-white/50 dark:text-gray-300 dark:hover:bg-gray-800/50 transition-colors duration-500 transition-ease-in-out'>
