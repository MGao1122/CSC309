...
const TemplatesIndex: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const pageFromQuery = parseInt(router.query.templatePage as string, 10);
        if (!isNaN(pageFromQuery) && pageFromQuery > 0) {
            setCurrentPage(pageFromQuery);
        }
    }, [router.query.templatePage]);

    const fetchTemplates = async (query = '', tags = '', page: number) => {
        try {
            const response = await axios.get(`http://localhost:3000/api/template?page=${page}&perPage=${12}`, {
    ...
    useEffect(() => {
        ...
        fetchTemplates(searchQuery, tagsParam, currentPage);
    }, [currentPage]);
