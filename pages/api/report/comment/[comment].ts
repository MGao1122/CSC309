...
import { findCommentById } from "@/utils/comments";
import { reportComment } from "@/utils/report";
...
async function handler(req: NextApiRequest & { userinfo?: { userId: number } }, res: NextApiResponse) {
    ...
    try {
        const com = await findCommentById(Number(comment));

        if (!com) {
            return res.status(404).json({ error: "Comment not found" });
        }

        if (com.visibility === false) {
            return res.status(403).json({ error: "Forbidden" });
        }

        const report = await reportComment(com.id, userId, content);
