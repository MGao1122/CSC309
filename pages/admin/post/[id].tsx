...
export default function PostReport() {
    ...
    const fetchReports = async () => {
        ...
        try {
            const response = await api.get(`/find/admin/post/${id}?page=${page}&perPage=8`);
    ...
    const ban = (visibility: boolean) => async () => {
        try {
            await api.put(`/admin/${reportId}`, { postVisibility: visibility });
    ...
    return (
        <div className="container mx-auto p-2">
            <Card className="p-8 container">
                ...
                <div className="m-4">
                    {post?.authorId !== undefined && <User id={post.authorId} />}
                    <h2 className="text-2xl font-semibold my-2">{post?.title}</h2>
                    <p className="text-lg text-gray-600 dark:text-white">{post?.description}</p>
                    <Button className="mt-4 bg-blue-500"
                        onClick={() => router.push(`/posts/auth/${id}`)}
                    >Post Link</Button>
                </div>
                ...
                {visibility && (
                    <>
                        ...
                        <p className="mt-4 text-lg text-gray-600 dark:text-white">This post is currently visible to users.</p>
                    </>
                )}
                {!visibility && (
                    <>
                        ...
                        <p className="mt-4 text-lg text-gray-600 dark:text-white">This post is currently hidden from users.</p>
                    </>
                )}
            </Card>
        </div>
